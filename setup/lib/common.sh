#!/usr/bin/env bash
# Private runtime implementation of the versioned setup package.
set -euo pipefail

AGENT_SETUP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
: "${AGENT_REPO_ROOT:?Pass the repository root to run.sh}"
AGENT_REPO_ROOT="$(cd "$AGENT_REPO_ROOT" && pwd -P)"
...
# Tool downloads are shared; activation belongs to this checkout, not the last
# repository that happened to prepare a different pinned runtime.
agent_sha256() { if command -v sha256sum >/dev/null 2>&1; then sha256sum; else shasum -a 256; fi; }
AGENT_CHECKOUT_KEY="$(printf '%s' "$AGENT_REPO_ROOT" | agent_sha256 | cut -d ' ' -f 1)"
AGENT_BIN="$AGENT_SETUP_HOME/checkouts/$AGENT_CHECKOUT_KEY/bin"
AGENT_ENV_FILE="$AGENT_SETUP_HOME/checkouts/$AGENT_CHECKOUT_KEY/env.sh"
...
agent_setup_fingerprint() {
  (
    cd "$AGENT_REPO_ROOT"
    cat "$AGENT_SETUP_DIR/package.json"
    if declare -F repo_fingerprint >/dev/null; then repo_fingerprint; fi
    printf '\0%s\0%s\0' "$AGENT_SETUP_BROWSER" "$AGENT_SETUP_ASTRALE_CLI"
...
agent_load_config() {
  local configuration="$AGENT_REPO_ROOT/scripts/setup/repo.sh" name
...
agent_ensure_bun() {
  if [[ -f "$AGENT_REPO_ROOT/.bun-version" ]]; then
    local version prefix
    version="$(tr -d '[:space:]' < "$AGENT_REPO_ROOT/.bun-version")"
    [[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || agent_die 'Invalid .bun-version'
    if [[ "$(bun --version 2>/dev/null || true)" != "$version" ]]; then
      [[ "$AGENT_SETUP_TOOLS" != check ]] || agent_die "Activate Bun $version locally"
      prefix="$AGENT_SETUP_HOME/bun/$version"
      if [[ "$("$prefix/bin/bun" --version 2>/dev/null || true)" != "$version" ]]; then
        agent_npm_install "$prefix" "bun@$version"
      fi
      agent_link "$prefix/bin/bun" bun
    elif [[ "$AGENT_SETUP_TOOLS" != check ]]; then
      agent_link "$(command -v bun)" bun
    fi
    [[ "$(bun --version)" == "$version" ]] || agent_die 'Incorrect Bun after preparation'
    return
  fi
  if [[ "$AGENT_SETUP_TOOLS" == check ]]; then
...
agent_environment_reference() {
  printf 'case "$PWD/" in %q/*) [ ! -f %q ] || . %q ;; esac # Astrale agent setup\n' "$AGENT_REPO_ROOT" "$AGENT_ENV_FILE" "$AGENT_ENV_FILE"
}
...
agent_persist_environment() {
  ...
  {
    printf '# Generated by Astrale setup: activation paths and non-secret tool selections.\n'
    if declare -F repo_environment >/dev/null; then repo_environment; fi
