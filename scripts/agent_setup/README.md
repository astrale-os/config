...
- **Codex Cloud:** select `astrale-os/config`, disable container caching, and put
  `AGENT_HARNESSES=codex bash scripts/agent_setup/setup.sh` in Setup script only.
- **Claude Cloud:** select `astrale-os/config` and leave Setup script empty. The committed
  ...
  use the [extended allowlist](https://github.com/astrale-os/domains/blob/main/scripts/agent_setup/README.md#cloud-and-conductor).
