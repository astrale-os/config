...
if [[ "${CLAUDE_CODE_REMOTE:-}" == true ]]; then
  ...
  mkdir -p "$state"
  # Keep one lock per physical checkout; readiness depends on preparation inputs.
  ...
  (
    ...
    if [[ -r "$AGENT_ENV_FILE" && -f "$marker" ]] && [[ "$(cat "$marker")" == "$fingerprint" ]]; then
      ...
      # A failed refresh must not leave a previous success eligible for reuse.
      rm -f "$marker"
      ...
      # Preparation may legitimately update a lockfile. Record the prepared state.
